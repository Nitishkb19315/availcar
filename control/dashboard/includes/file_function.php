...
////   file uploading functions
	...
	function update_profile($tmp_name, $file_name, $upload_dir,$width,$height, $name){
	  		$target_file = get_target_file_update_profile($file_name, $name);
	  		if(delete_photo($upload_dir, $file_name) && !empty($target_file) || !$target_file){
	  			$upload_dir = "../uploads/".$upload_dir;
	  			if(move_uploaded_file($tmp_name,$upload_dir."/".$target_file)){
	  				thumbnail( $upload_dir."/".$target_file, $upload_dir."/".$target_file, $width, $height);
					return $target_file;
	...
	function get_target_file_update_profile($file_name, $name){
		$target_file="";
		$info = new SplFileInfo($file_name);
		$extension = $info->getExtension();
        ...
        $filename= basename($_POST['old_profile_image'],".".$extension);
	...
	function delete_additional_image($dir_type, $images){
		for($i=0; $i < count($images); $i++){
			if(!delete_photo($dir_type, $images[$i])){
				return false;
			}
		}
		return true;
